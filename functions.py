...
def maxval(l,i):
    return max(enumerate(l1[i] for l1 in l), key=itemgetter(1))
...
def weighted_modularity(G, Gu, data):
    part = community.best_partition(Gu)
    mod = community.modularity(part, Gu, weight = 'weight')

    return mod

def directed_modularity(G, Gu, data):
    part = community.best_partition(Gu)
    ...
    for i in range(n):
        for j in range(n):
            if (i != j):
                ans += ((mat[i][j] - ((outd[i] * ind[i]) / l)) * delta[i][j])
...
def weighted_closeness_centrality(G):
    return (1/weighted_characteristic_path_length(G))

def directed_closeness_centrality(G):
    return (1/directed_characteristic_path_length(G))
...
'''
def weighted_participation_coefficient(G, data):
    s = 1
    #TO DO
    return s
'''

def plot_graph_save(G, path, outfile1, outfile2, title):
    fig1 = plt.figure(1)
    ...
    fig1.savefig(path+outfile1)
    A = nx.adjacency_matrix(G)
    plt.matshow(A.todense(), cmap=plt.cm.Blues)
    plt.title(title)
    plt.savefig(path+outfile2)
    fig1.clear()
    print "images saved"
    #plt.show()

def plot_graph_save_undirected(G, path, outfile1, outfile2, title):
    fig1 = plt.figure(1)
    G = nx.Graph(G)
    pos = nx.random_layout(G)  # fruchterman_reingold_layout, shell_layout, circular_layout, spring_layout, circular_layout, random_layout
    colors = [d['weight'] for (u, v, d) in G.edges(data=True)]
...
def plot_graph(G, path, outfile1, outfile2, title):
    fig1 = plt.figure(1)
    pos = nx.random_layout(G)  # fruchterman_reingold_layout, shell_layout, circular_layout, spring_layout, circular_layout, random_layout
    colors = [d['weight'] for (u, v, d) in G.edges(data=True)]
    nx.draw(G, pos, node_color='#A0CBE2', edge_color=colors, edge_cmap=plt.cm.Blues, with_labels=True)
...
def grangerCausality(data, k ,l, lag_order):
    testarray = data[:, [k,l]]
    gr = tsa_stats.grangercausalitytests(testarray[:, 1::-1], lag_order, verbose=False)
    pval = gr[lag_order][0]['ssr_ftest'][1]
    stats_val = gr[lag_order][0]['ssr_ftest'][0]
